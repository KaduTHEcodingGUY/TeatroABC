...
public class JavaFxteste {
    ...
    public Parent getRoot(Stage primaryStage) {
        ...
        BorderPane sidebar = createSidebar(primaryStage); 
        rootLayout.setLeft(sidebar);

        setCenterView(clubeFidelidadeViewProvider.getView());
    ...
    private BorderPane createSidebar(Stage primaryStage) { 
        ...
        // --- MODIFICADO: Criando botões com o novo método auxiliar ---
        Button btnComprar = createNavButtonWithIcon("Comprar Ingressos", "/Utilitarios/ticket.png", null);
        Button btnMeusIngressos = createNavButtonWithIcon("Meus Ingressos", "/Utilitarios/tickets.png", null);
        Button btnClube = createNavButtonWithIcon("Clube Fidelidade", "/Utilitarios/club (1).png", clubeFidelidadeViewProvider);
        ...
        Label userName = new Label("Alex Johnson");
    ...
    /**
     * NOVO MÉTODO AUXILIAR: Cria um botão de navegação completo com texto, ícone e interações.
     */
    private Button createNavButtonWithIcon(String text, String iconPath, ProvedorView provider) {
        ...
        button.setPrefWidth(200); // Aumenta um pouco a largura para caber o ícone
        button.setGraphicTextGap(15); // Espaçamento entre o ícone e o texto
        button.setPadding(new Insets(8, 12, 8, 12)); // Padding interno
        button.setContentDisplay(ContentDisplay.LEFT); // Garante que o ícone fique à esquerda
        ...
        button.setOnAction(event -> {
            if (provider != null) {
                ...
                botaoSelecionado.setBackground(BG_BOTAO_SELECIONADO);
                setCenterView(provider.getView());
            } else {
